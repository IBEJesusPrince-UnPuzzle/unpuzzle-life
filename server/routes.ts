...
export function registerRoutes(server: Server, app: Express) {
  ...
  // Travel Mode Sync migration — idempotent, benign if column already exists.
  try {
    sqlite.exec(`ALTER TABLE users ADD COLUMN timezone TEXT`);
  } catch (_) { /* column already exists — expected on subsequent boots */ }
  ...
  // TIMEZONE (Travel Mode Sync)

  // ============================================================

  // Lightweight endpoint called on every app open. Persists the device's
  // IANA timezone string so backend schedulers can fire push notifications
  // at the correct physical local time after the user travels.
  app.patch("/api/user/timezone", (req, res) => {
    ...
    const userId = Number(getEffectiveUserId(req));

    sqlite.prepare(`UPDATE users SET timezone = ? WHERE id = ?`).run(timezone, userId);
