...
- Name: notifications.spec.ts >> Notification Preferences >> should persist notification preferences after navigation
- Location: e2e\notifications.spec.ts:21:3
...
```ts
  ...
  2   | 
  3   | test.describe('Notification Preferences', () => {
  4   |   test.beforeEach(async ({ page }) => {
  5   |     // Navigate to the app and login
  6   |     await page.goto('/');
  7   |     await page.waitForLoadState('networkidle');
  8   |     
  9   |     // Login if needed (adjust based on your auth flow)
  10  |     const loginButton = page.locator('button, a').filter({ hasText: /login|sign in/i }).first();
  11  |     if (await loginButton.isVisible()) {
